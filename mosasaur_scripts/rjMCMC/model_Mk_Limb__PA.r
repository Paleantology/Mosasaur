    ...
    n_max_states <- 7
     ...
     limb_partition <- append(cranium, core)
     limb_partition<- append(limb_partition, tooth)
     
     morpho_limb_partition <- morpho
     morpho_limb_partition.excludeCharacter(limb_partition)
     morpho_limb_partition
      for (i in 1:n_max_states) {
        morpho_limb_partition_bystate[i] <- morpho_limb_partition
        morpho_limb_partition_bystate[i].setNumStatesPartition(i)
         nc = morpho_limb_partition_bystate[i].nchar()
     ...
     # create model of evolution for the character block
             ...
             # attach the data
     	    m_morph[idx].clamp(morpho_limb_partition_bystate[i])
             idx = idx + 1
     idx
     ...
     morpho_everything_else.excludeCharacter(limb)
     morpho_everything_else
      for (i in 1:n_max_states) {
        ...
        morpho_everything_else_bystate[i].setNumStatesPartition(i)
         nc = morpho_everyhting_else_bystate[i].nchar()
         # for non-empty character blocks
         if (nc > 0) {
             # make i-by-i rate matrix
             q[idx] <- fnJC(i)
